...
final class Create extends Component
{
    ...
    {
        return match (true) {
            filled($this->parentId) => __('Write a comment...'),
            $this->isSharingUpdate() => __('hare an update...'),
            default => __('Ask a question...')
        };
    }
    ...
    public function store(Request $request): void
    {
        ...
        if (! app()->isLocal() && $user->questionsSent()->where('created_at', '>=', now()->subMinute())->count() >= 3) {
            $this->addError('content', __('You can only send 3 questions per minute.'));

            return;
        }

        if (! app()->isLocal() && $user->questionsSent()->where('created_at', '>=', now()->subDay())->count() > 30) {
            $this->addError('content', __('You can only send 30 questions per day.'));
        ...
        $message = match (true) {
            filled($this->parentId) => __('Comment sent.'),
            $this->isSharingUpdate => __('Update sent.'),
            default => __('Question sent.')
        };
    ...
    public function uploadImages(): void
    {
        collect($this->images)->each(function (UploadedFile $image): void {
            ...
            /** @var string $path */
            $path = $image->store("images/questions/{$today}", 'public');
            ...
            if ($path) {
                ...
                $this->addError('images', __('The image could not be uploaded.'));
                $this->dispatch('notification.created', message: __('The image could not be uploaded.'));
