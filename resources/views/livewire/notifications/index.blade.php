<div class="mb-20 flex flex-col gap-2">
                ...
                @if (! $isMention)
                    @if ($question->parent_id !== null)
                        <div class="flex items center gap-3 text-sm text-slate-500">
                            ...
                            <p>{{ $question->from->name }} {{ __('commented on your') }} {{ $question->parent->parent_id !== null ? 'comment' : ($question->parent->isSharedUpdate() ? __('Update') : __('Answer') ) }}:
                        </div>
                    @elseif ($question->from->is(auth()->user()) && $question->answer !== null)
                        <div class="flex items-center gap-3 text-sm text-slate-500">
                            ...
                            <p>{{ $question->to->name }} {{ __('answered your') }} {{ $question->anonymously ? 'anonymous' : '' }} {{ __('question') }}:</p>
                        </div>
                    @else
                        @if ($question->anonymously)
                            <div class="flex items-center gap-3 text-sm text-slate-500">
                                ...
                                <p>{{ __('Someone asked you anonymously:') }}</p>
                            </div>
                        @else
                            <div class="flex items-center gap-3 text-sm text-slate-500">
                                <figure class="{{ $question->from->is_company_verified ? 'rounded-md' : 'rounded-full' }} h-10 w-10 flex-shrink-0 dark:bg-slate-800 bg-slate-50 transition-opacity group-hover:opacity-90">
                                ...
                                <p><span class="dark:text-white text-black">{{ $question->from->name }}</span> {{ __('asked you:') }}</p>
                            </div>
                        @endif
                    @endif
                @elseif ($question->parent !== null)
                    <p class="text-sm text-slate-500">{{ __('You have been mentioned in a comment by') }} {{ '@' . $question->to->username }}</p>
                @else
                    <p class="text-sm text-slate-500">{{ __('You have been mentioned in a') }} {{ $question->isSharedUpdate() ? 'update by @'.$question->to->username : 'question:'}}</p>
                @endif
    ...
    @if ($notifications->isEmpty())
        <div class="rounded-lg">
            <p class="text-slate-400">{{ __('No pending notifications.') }}</p>
        </div>
    @endif
</div>
