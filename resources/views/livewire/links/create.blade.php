<form wire:submit="store" x-data="imageUpload"
    ...
    <div class="space-y-3">
        <div>
            ...
            <x-text-input id="title" type="text" wire:model="title" class="mt-1 block w-full" required
                autofocus />
            @error('title')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <div wire:ignore id="linkCreateMap" class="w-full h-[25vh] rounded-lg mt-1"></div>
                @error('lng')
                    <x-input-error :messages="$message" class="mt-2" />
                @enderror
            </div>
            ...
            <div class="flex justify-between gap-2 mt-10">
                <div class="flex w-full basis-3/5">
                    <div>
                        ...
                        <span class="text-xs text-slate-600">{{ __('Maximum :number photos',['number' => $uploadLimit]) }}</span>
                    </div>
                </div>
                ...
                <button title="{{ __('Add a photo') }}" x-ref="imageButton"
                    ...
                    <template x-if="uploading" >
                        <div class="cursor-wait inline-flex items-center gap-2 whitespace-nowrap" :disabled="uploading || images.length >= uploadLimit">
                    ...
                    <template x-if="!uploading">
                        <div class="inline-flex items-center gap-2 whitespace-nowrap" :disabled="uploading || images.length >= uploadLimit">
        ...
        <div class="flex items-center gap-4 border-t border-t-slate-600 pt-3">
            <x-primary-colorless-button class="text-teal-500 border-teal-500"
                type="submit">
                {{ __('Add') }}
...
@push('scripts')
    ...
    <script>
        window.addEventListener('linkCreateMap', e => {
            
          //  var images = @this.images;
            mapboxgl.accessToken = '{{ $accessToken }}';
            const map = new mapboxgl.Map({
                container: 'linkCreateMap',
