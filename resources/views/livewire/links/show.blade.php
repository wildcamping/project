<article class="block" id="q-{{ $linkId }}" >
    <div 
        class="">
        <div class="pb-3 border-b dark:border-slate-900 border-slate-200">
            <div class="flex justify-between border-b dark:border-slate-900 border-slate-200 pb-3">
                <a href="{{ route('profile.show', ['username' => $link->user->username]) }}"
                    class="group/profile flex items-center gap-3" data-navigate-ignore="true" wire:navigate>
                    <figure
                    ...
                    <div class="overflow-hidden text-sm">
                        <div class="items flex">
                            ...
                            @if ($link->user->is_verified && $link->user->is_company_verified)
                                <x-icons.verified-company :color="$question->user->right_color" class="ml-1 mt-0.5 h-3.5 w-3.5" />
            ...
            <h6 class="text-slate-800 dark:text-slate-400 font-semibold mt-3 ">
                {{ $link->title }}
            </h6>
            ...
            <div class="flex flex-row justify-end gap-4 mt-4">
                @foreach ($property as $item)
                    @if ($item['slug'] == 'altana')
                        <x-popover text="{{ $item['name'] }}: {{ $item['value'] == 1 ? 'jest' : 'brak' }}"
        ...
        <div class="mt-6 flex items-center justify-end text-sm text-slate-500 gap-4" x-data="clickHandler">


            <x-primary-button class="gap-2" x-on:click="navigate('{{ $link->lat }}', '{{ $link->lng }}')">
                <x-heroicon-o-truck class="size-4" />{{ __('Nawiguj') }}
            </x-primary-button>
            <x-primary-button class="gap-2" as="a" wire:navigate
                href="{{ Route('links.show', [
                    'link' => $link->id,
                    'username' => $link->user->username,
                ]) }}">
                <x-heroicon-o-ellipsis-horizontal-circle class="size-4" />{{ __('Więcej') }}
            </x-primary-button>
        </div>
    </div>
</article>
