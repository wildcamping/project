<div
    ...
    <div class="relative bg-gradient-to-r p-5 text-center dark:text-white text-black">
        <div class="absolute left-0 top-6 flex">
            <x-dropdown-link-profile>
                <x-slot name="trigger">
                    <button
                        x-bind:class="{
                            ...
                            'dark:bg-slate-900 bg-slate-50 dark:hover:bg-slate-800 hover:bg-slate-100 border dark:border-transparent border-slate-200':
                                !open
                        }"
                ...
                <x-slot name="content">
                    <button x-data="shareProfile" x-show="isVisible"
                    ...
                    <button x-data="copyUrl" x-show="isVisible"
                        x-on:click="
                            copyToClipboard(
            ...
            @if (auth()->user()?->is($user))
                <button
                    ...
                    <span class="sr-only">{{ __('See QR Code') }}</span>
        ...
        @if (!$user->is(auth()->user()))
            <div class="absolute right-0 top-6 flex">
                @if ($user->followers()->where('follower_id', auth()->id())->exists())
                    <button type="button" wire:click="unfollow({{ $user->id }})"
                ...
                @else
                    <button type="button" wire:click="follow({{ $user->id }})"
                        class="flex items-center justify-center rounded-lg border dark:border-transparent border-slate-200 dark:bg-slate-900 bg-slate-50 px-2 py-1 dark:text-slate-300 text-slate-600 transition duration-150 ease-in-out dark:hover:bg-slate-800 hover:bg-slate-200 dark:hover:text-white hover:text-black">
                        {{ __('Follow') }}
                    </button>
                @endif
        ...
        <div class="relative mx-auto h-24 w-24" x-data="{ showAvatar: false }">
            ...
            <div x-show="showAvatar" x-cloak x-on:click="showAvatar = false"
            ...
            @if (auth()->user()?->is($user))
                <button
                    ...
                    href="{{ route('profile.edit') }}" wire:navigate title="{{ __('Upload Avatar') }}">
        ...
        <a class="dark:text-slate-400 text-slate-600"
            href="{{ route('profile.show', ['username' => $user->username]) }}" wire:navigate>
        ...
        @elseif (auth()->user()?->is($user))
            <a href="{{ route('profile.edit') }}" class="text-sm text-slate-500 hover:underline"
                wire:navigate>{{ __('Tell people about yourself') }}</a>
        @endif
        ...
        <div class="mt-2 text-sm">
            <p class="dark:text-slate-400 text-slate-600">
                @if ($user->followers_count > 0)
                    <button x-on:click.prevent="$dispatch('open-modal', '{{ __('followers') }}')">
                        <span class="cursor-help"
                            title="{{ Number::format($user->followers_count) }} {{ str(__('Follower'))->plural($user->followers_count) }}">
                            {{ Number::abbreviate($user->followers_count) }}
                            {{ str(__('Follower'))->plural($user->followers_count) }}
                        </span>
                    </button>
                ...
                @if ($user->following_count > 0)
                    <button x-on:click.prevent="$dispatch('open-modal', 'following')">
                        <span class="cursor-help"
                            title="{{ Number::format($user->following_count) }} {{ __('Following') }}">
                ...
                @if ($questionsReceivedCount > 0)
                    <span class="cursor-help"
                        title="{{ Number::format($questionsReceivedCount) }} {{ str(__('Post'))->plural($questionsReceivedCount) }}">
                        {{ Number::abbreviate($questionsReceivedCount) }}
                        {{ str(__('Post'))->plural($questionsReceivedCount) }}
                    </span>
                ...
                <span class="cursor-help"
                    title="{{ Number::format($user->views) }} {{ str(__('View'))->plural($user->views) }}">
                    {{ Number::abbreviate($user->views) }} {{ str(__('View'))->plural($user->views) }}
                </span>
            </p>
        </div>
    </div>
    <div class="py-5">
        @if ($links->isEmpty())
            @if (auth()->user()?->is($user))
                <p class="mx-2 text-center text-slate-500">{{ __('No links yet. Add your first link!') }}</p>
            @endif
        @else
            @if (auth()->user()?->is($user))
                <ul x-data="{ isDragging: false }" x-sortable x-on:choose.stop="isDragging = true"
                    x-on:unchoose.stop="isDragging = false" wire:end.stop="storeSort($event.target.sortable.toArray())"
                    class="space-y-3">
                    @foreach ($links as $link)
                        <li class="relative h-12 hover:darken-gradient group flex {{ $link->is_visible ? 'bg-gradient-to-r' : 'bg-gray-500' }} overflow-hidden"
                            :class="showSettingsForm ? gradient + ' ' + link_shape :
                                '{{ $user->gradient }} {{ $user->link_shape }}'"
                            x-sortable-item="{{ $link->id }}" wire:key="link-{{ $link->id }}"
                            x-data="{ showActions: false }" x-on:click.outside="showActions = false">
                            ...
                            <div x-on:click="showActions = !showActions" x-bind:class="{ 'invisible': isDragging }"
                                ...
                                <x-heroicon-o-chevron-double-left
                                    ...
                                    x-bind:class="{ 'rotate-180': showActions }" x-cloak />
                            </div>
                            ...
                            <div class="absolute -right-56 top-0 bottom-0 flex items-center justify-center transition-all duration-500 z-5"
                                ...
                                <div class="text-white min-w-fit cursor-help items-center gap-1 text-xs"
                                    title="{{ __('Clicked :number times', ['number' => Number::format($link->click_count)]) }}"
                                    x-bind:class="{ 'invisible': isDragging }">
                                    {{ Number::abbreviate($link->click_count) }}
                                    {{ str(__('click'))->plural($link->click_count) }}
                                </div>

                                <button wire:click="setVisibility({{ $link->id }})" type="button"
                                ...
                                <form wire:submit="destroy({{ $link->id }})">
                                    <button
                                        onclick="if (!confirm(__('Are you sure you want to delete this link?'))) { return false; }"
                                        type="submit"
                                        class="flex w-10 justify-center text-slate-50 opacity-50 hover:opacity-100 focus:outline-none">
                                        <x-heroicon-o-trash
                    ...
                    <a class="flex w-full basis-2/5 items-bottom justify-end px-4 py-2 text-sm"
                        href="{{ route('profile.links.index') }}">
                        {{ __('Show more ...') }}
                    </a>
                    </div>
            ...
            @else
                <div class="space-y-3">
                    @foreach ($links as $link)
                        <div class="{{ $user->link_shape }} {{ $user->gradient }} h-12 hover:darken-gradient flex justify-center bg-gradient-to-r"
        ...
        <div x-data="{
            ...
            <div>
                <div class="flex gap-2">
                    <button wire:click="$dispatchTo('links.create', 'link.create', {  })"
                    ...
                    <button x-on:click="showSettingsForm = ! showSettingsForm ; showLinksForm = false"
                        ...
                        :class="showSettingsForm ? 'bg-' + gradient.split(' ')[1].replace('to-', '') + ' ' + link_shape :
                            'bg-{{ $user->right_color }} {{ $user->link_shape }}'">
                ...
                <div x-show="showSettingsForm" x-transition:enter="transition duration-300 ease-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="transition duration-300 ease-in" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0" class="mx-2 mt-4" x-cloak>
