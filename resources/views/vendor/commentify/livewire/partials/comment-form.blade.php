<form class="mb-6" wire:submit="{{ $method }}">
    ...
    <div
        ...
        <textarea id="{{ $inputId }}" rows="6"
            ...
            placeholder="{{ __('Write a comment...') }}" wire:model.live="{{ $state }}.body" oninput="detectAtSymbol()"></textarea>
...
{{--
    ...
    --}}
    <x-primary-button wire:loading.attr="disabled" class="gap-2">
        <x-heroicon-o-chat-bubble-bottom-center-text class="size-4" />
        <div wire:loading wire:target="{{ $method }}">
            @include('commentify::livewire.partials.loader')
        </div>
        {{ $button }}
